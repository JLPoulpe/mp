...
class OrderControllerCore extends ParentOrderController
{
    ...
    protected function _applyTypeOrderRule() {
        ...
        foreach($listProducts as $cartDto) {
            ...
            if($productDto->getCategoryId()==Category::CATEGORY_PANIER) {
                $panier = true;
                $dateT = MPTools::getNextDateFromDay($productDto->getReference());
                $panierJour[$productDto->getReference()] = MPTools::$listJour[$dateT->format('w')] . $dateT->format(' d') . ' ' . MPTools::$listMois[$dateT->format('n')-1] . $dateT->format(' Y');
            } else {
                $produit = true;
                if(empty($date)) {
                    $date = $cartDto->getDateWithdrawal();
                } elseif ($date!=$cartDto->getDateWithdrawal()) {
                    $manyDays = true;
                }
            }
        }
        ...
        $this->context->smarty->assign(
            array(
                'panier'        => $panier,
                'panierJour'    => $panierJour,
                'produit'       => $produit,
                'manyDays'      => $manyDays,
                'retraitmarche' => Carrier::RETRAIT_SUR_MARCHE,
            )
        );
    }
}
