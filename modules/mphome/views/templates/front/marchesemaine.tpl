                ...
                <div class="date">{$jour.displayDate}</div>
                 <div class="jour">
                    {foreach $jour.marketDto as $marketDto}
                        <a href="/marches-locaux/{$marketDto->getCityRewrite()}/{$marketDto->getPostalCode()}/{$marketDto->getIdMarket()}/{$marketDto->getLinkRewrite()}/jour/{$jour.jour}" class="marche">
                            <div class="city">{$marketDto->getCity()}</div>
                            <div class="name">{$marketDto->getName()}</div>
                        </a>
                    {/foreach}
