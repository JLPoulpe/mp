...
function addMPCommission(price, pourcentage)
{
    return price * (1 + pourcentage / 100);
}
...
function calcPrice()
{
   ...
   var prixPaysansTTC = parseFloat(document.getElementById('wholesale_price').value.replace(/,/g, '.'));
   var prixPaysansHT = removeTaxes(prixPaysansTTC);
   var mpCom = parseFloat(document.getElementById('mp_com').value.replace(/,/g, '.'));
   if(isNaN(mpCom)) {
       mpCom = 0;
   }
   var prixMPHT = addMPCommission(prixPaysansHT, mpCom);
   var prixMPTTC = addTaxes(prixMPHT);
   document.getElementById('unit_price').value = (isNaN(prixMPHT) == true || prixMPHT < 0) ? '' :
		ps_round(prixMPHT, priceDisplayPrecision);
   document.getElementById('finalPrice').innerHTML = (isNaN(prixMPTTC) == true || prixMPTTC < 0) ? '' :
		ps_round(prixMPTTC, priceDisplayPrecision).toFixed(priceDisplayPrecision);
	document.getElementById('finalPriceWithoutTax').innerHTML = (isNaN(prixMPHT) == true || prixMPHT < 0) ? '' :
		(ps_round(prixMPHT, 6)).toFixed(6);
}
...
function calcPriceTI()
{
	...
	var mpCom = document.getElementById('mp_com').value;
    var newPrice = addMPCommission(priceTE, mpCom);
    ...
    document.getElementById('unit_price').value = (isNaN(newPrice) == true || newPrice < 0) ? '' :
		ps_round(newPrice, priceDisplayPrecision);
