...
class CartCore extends ObjectModel
{
	...
	public function getProducts($refresh = false, $id_product = false, $id_country = null)
	{
		...
		// Build SELECT
		$sql->select('cp.`id_product_attribute`, cp.`date_withdrawal`, cp.`id_product`, cp.`quantity` AS cart_quantity, cp.id_shop, pl.`name`, p.`is_virtual`,
			...
			foreach ($result as $key => $row)
			{
				...
				if ($specific_price)
					...
					$reduction_type_row = array('reduction_type' => 0);
                                $dateT = new DateTime($row['date_withdrawal']);
                                $shippingDate = array('shippingDate'=>$dateT->format('d/m/Y'));
				$result[$key] = array_merge($row, $reduction_type_row, $shippingDate);
			}
